---
title: "README-RMARKDOWN"
author: "Frank"
date: "2020/5/14"
output: md_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(lazy.work)
```

# lazy.work
Some R functions to save your time at Work

## Install

```r
devtools::install_github("frankzhangsyd/lazy.work")
```

## Main Usages

### Alteryx Related

**ayx_fl**:convert a vector of column names and a function symbol`("+","-","*","/")` across all.

ie: 

```{r}
ayx_fl(c("a","b"))

```

### Manipulation of files

**file_copy2_folder** :copy files or files in a folder to another place with adding prefix or suffix.

ie:

```r
file_copy2_folder(from = "my folder path",to = c("destination_folder1","destination_folder2"),affix_type = "prefix",affix = "Prefix_",from_is_folder = TRUE)

```

**file_cut2_folder** :Copy files or files in a folder to another place with adding prefix or suffix

ie:

```r
file_cut2_folder(from = "my folder path",to = "destination_folder1",affix_type = "prefix",affix = "Prefix_",from_is_folder = TRUE)

```

